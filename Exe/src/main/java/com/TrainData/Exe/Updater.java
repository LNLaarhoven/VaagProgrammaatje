...
public class Updater extends TimerTask {
	...
	private Trein addNewTrain(Arrivals arrival) {
		...
		if (LIVE) {
			...
			CRUDPostTreinNaarDatabase(trein);
				...
				try {
				System.out.println("Trein "+trein.getNaam()+" zat al in de database. Wordt nu geupdate ipv aangemaakt.");
				CRUDPutUpdateTreinInDatabase(trein);
				} catch(HttpClientErrorException n) {
					System.out.println("Trein "+trein.getNaam()+", dit lukt niet om te updaten maar dit lukte ook niet!");
				}
			}
		}
		return trein;
	...
	private void CRUDPutUpdateTreinInDatabase(Trein trein) {
		// HTTP PUT REQUIREMENTS
		HttpEntity<Trein> entity = new HttpEntity<Trein>(trein);
		RestTemplate restTemplate = new RestTemplate();
		restTemplate.exchange(databaseUrl+"/"+trein.getNaam(), HttpMethod.PUT, entity, Trein.class);
	}
}